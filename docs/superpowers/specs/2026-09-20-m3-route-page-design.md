...
- `/route?goal=salmon` draws the fixtures' 17 stops as **five** lines converging on Salmon, with
  the List view beside it;
...
| **y — the band** | its **region**, in `regions.yaml` order; one band per region the route touches — five for Salmon, since *Algorithms* sits below the route, not on it |
...
**Colour stays lawful.** W10 gives one meaning per colour, so the regions cannot be five colours:
...
- replaces the 13 invented stops with the fixtures' **17**, across their **five real regions**,
...
| Test | Proves |
|---|---|
| 17 stops in five bands, in `regions.yaml` order | lines are regions |
...
| the 14-stop `known` route lays out with three stops fewer | known shortens the picture |
