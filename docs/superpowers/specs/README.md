...
| [2026-09-17 — M0 part 9: repository guardrails](2026-09-17-m0-repository-guardrails-design.md) | agreed; built | a `validate` CI stub, squash-only auto-merge and a no-bypass ruleset on `comeni-code-content`; a no-bypass ruleset requiring `python`, `web` and `stack` on Comeni-Code; rulesets kept as files and applied only after the operator confirms |
| [2026-09-17 — M0 part 8: the Compose stack](2026-09-17-m0-compose-stack-design.md) | agreed; built | one API image for migrate, api (gunicorn), worker and beat; nginx serving the built web app on 127.0.0.1:8090; health checks that gate startup (API serving = 200 or 503); `ops/stack-check.sh` through web; a CI `stack` job |
| [2026-09-17 — M0 part 7: the health page](2026-09-17-m0-health-page-design.md) | agreed; built | health at `/` and the specimen at `/identity` (no router until M3), Vite's `/api` proxy via `CODE_WEB_API_ORIGIN`, API types from `openapi.json` with json-schema-to-typescript (TypeScript 7 has no JS API for openapi-typescript) and a drift test, five worded states polled every 10 s |
