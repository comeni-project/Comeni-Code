...
| [2026-09-17 — M0 part 8: the Compose stack](2026-09-17-m0-compose-stack-design.md) | agreed | one API image for migrate, api (gunicorn), worker and beat; nginx serving the built web app on 127.0.0.1:8090; health checks that gate startup (API serving = 200 or 503); `ops/stack-check.sh` through web; a CI `stack` job |
